...
class CAVEnv(MarkovGameEnv):
    ...
    def __init__(self, bodies, constants, env_config, np_random=seeding.np_random(None)[0]):
        ...
        # print("self.ego.constants.max_velocity %d" % self.ego.constants.max_velocity)
        # print("self.ego.constants.min_velocity %d" % self.ego.constants.min_velocity)
        # print("delta left                      %d" % (self.ego.constants.max_velocity - self.ego_maintenance_velocity))
        # print("delta right                     %d" % (self.ego.constants.min_velocity - self.ego_maintenance_velocity))
    ...
    def step(self, joint_action):
        ...
        # joint_reward is a list of rewards based on the environment
        joint_reward = [0.0 for _ in self.bodies] # underscore (_) ignores the value when unpacking
        ...
        # print("ego maintenance velocity %9.3f" % self.ego_maintenance_velocity)
        # print("ego max velocity offset %9.3f" % self.ego_max_velocity_offset)
        # print("ego relative offset %9.3f" % ego_velocity_relative_offset)
        # print("self.env_config.cost_step %d" % self.env_config.cost_step)
        # print("joint reward product %9.3f" % (ego_velocity_relative_offset * self.env_config.cost_step))
        # print("ego joint_reward %9.3f" % joint_reward[0])
        ...
        for i, polygon in enumerate(body_polygons):
            if i > 0:
                ...
                joint_reward[i] += ego_relative_distance_to_destination * self.env_config.cost_step #KM Edit
